<?php
            ...
            if(isset($_POST['name']) && $_POST['name']!=""){
                $name=$_POST['name'];
                $sql="SELECT * FROM `purchase` WHERE `pending` = 1 AND (`invoice_no` LIKE '$name%' OR `name` LIKE '$name%' OR `date` LIKE '$name%') ORDER BY `date` desc";
                $result=mysqli_query($conn,$sql);
                while($row=mysqli_fetch_array($result)){
                    $invoice=$row['invoice_no'];
                    $pending_amount=0;
                    $sql3="SELECT * FROM `customers_payment_details` WHERE `invoice_no` = '$invoice' AND `type` = 'p'";
            ...
            $sql="SELECT * FROM `purchase` WHERE `pending` = 1 ORDER BY `date` desc";
            $result=mysqli_query($conn,$sql);
            while($row=mysqli_fetch_array($result)){
            $invoice=$row['invoice_no'];
            $pending_amount=0;
